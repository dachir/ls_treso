...
def _make_auto_invoice_payment_entry(doc, invoice_name, config, caisse_account):
    """
    Payment Entry for an invoice automatically generated by LS Tréso.

    Same rule:
        Encaissement -> Sales Invoice -> Receive
        Decaissement -> Purchase Invoice -> Pay

    Invoice amount is in the transaction currency.
    party_amount is in the party-account currency.
    bank_amount is in the cash-account currency.
    """
    ...
    # ----------------------------------------------------------
    # Devise du compte tiers
    # Customer -> debit_to
    # Supplier -> credit_to
    # ----------------------------------------------------------

    party_currency = _get_party_account_currency(
        invoice,
        config,
    )

    # ----------------------------------------------------------
    # Montant à solder côté tiers.
    #
    # invoice.outstanding_amount est dans la devise de facture.
    # On le convertit dans la devise du compte tiers en utilisant
    # prioritairement le taux de la facture.
    # ----------------------------------------------------------

    party_amount = _convert_amount(
        doc,
    ...
    # ----------------------------------------------------------
    # Montant réellement reçu/payé par la caisse.
    # ----------------------------------------------------------
    ...
    #
    # party_amount :
    #     devise du compte Customer/Supplier
    #
    # bank_amount :
    #     devise de la caisse
    #
    # En passant explicitement party_amount, on évite
    # l'ambiguïté multi-devise de get_payment_entry().
    # ----------------------------------------------------------
    ...
    # pe.set("references", [])
    #
    # Ne pas recalculer allocated_amount.
    #
    # get_payment_entry() a reçu party_amount dans la bonne devise
    # et construit la référence en conséquence.
