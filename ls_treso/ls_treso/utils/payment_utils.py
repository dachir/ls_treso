...
def prepare_operation(doc):
    ...
    # Par défaut, on considère que l'opération suivra
    # le flux général.
    doc.flags.ls_treso_auto_invoice_only = False

    # Crée éventuellement la facture manquante.
    invoice = create_missing_invoices(doc)

    # ----------------------------------------------------------
    # CAS SIMPLE : FACTURE AUTOMATIQUE
    # ----------------------------------------------------------
    #
    # Pour utiliser le chemin simplifié :
    #
    # - une facture vient réellement d'être créée ;
    # - aucune ancienne avance n'est utilisée ;
    # - aucune ligne Employee ;
    # - aucune Nature avance ;
    # - aucune opération spéciale ;
    # - toutes les lignes normales appartiennent à
    #   la facture créée.
    #
    # Dans ce cas, inutile de passer par get_invoice_rows()
    # et ses conversions.
    # ----------------------------------------------------------

    if invoice and not doc.advance_allocation:
        ...
        for row in doc.details_operation_de_caisse or []:
            ...
            if row.invoice != invoice.name:
                auto_invoice_only = False
                break

        if has_normal_row and auto_invoice_only:
            doc.flags.ls_treso_auto_invoice_only = True
            return
...
def _make_auto_invoice_payment_entry(
    doc,
    invoice_name,
    config,
    caisse_account,
    ...
    """
    Create the Payment Entry for an invoice automatically
    generated by LS Tréso.
    ...
    - transaction amount comes from Encaissement / Decaissement;
    - transaction currency comes from Encaissement / Decaissement;
    - invoice is created in that same currency;
    - cash account must use that same currency;
    - allocated_amount comes from ERPNext's own current
      outstanding calculation.
    """

    # Import local volontaire :
    # aucun changement nécessaire dans les imports du fichier.
    ...
    if invoice.docstatus != 1:
        frappe.throw(
            _("La facture {0} doit être soumise").format(
                invoice_name
            )
        )
    ...
    # ----------------------------------------------------------
    # SOURCE DE VÉRITÉ LS TRÉSO
    # ----------------------------------------------------------

    transaction_amount = flt(doc.montant)
    transaction_currency = doc.devise
    ...
    if not transaction_currency:
        frappe.throw(
            _("La devise de l'opération est obligatoire")
        )
    ...
    if invoice.currency != transaction_currency:
        frappe.throw(
            _(
                "La facture {0} est en {1} alors que l'opération "
                "LS Tréso est en {2}"
            ).format(
                invoice.name,
                invoice.currency,
    ...
    # Une facture automatique doit représenter exactement
    # le montant de l'opération.
    if abs(invoice_amount - transaction_amount) > 0.01:
        frappe.throw(
            _(
                "Le montant de la facture automatique {0} ({1} {2}) "
                "est différent du montant de l'opération ({3} {2})"
            ).format(
                invoice.name,
                invoice_amount,
                transaction_currency,
    ...
    if not caisse_currency:
        frappe.throw(
            _("Impossible de déterminer la devise de la caisse")
        )

    # Règle métier :
    #
    # 500 000 CDF encaissés
    # -> facture CDF
    # -> caisse CDF
    # -> Payment Entry CDF côté caisse
    #
    # Le compte client/fournisseur peut rester dans une autre
    # devise grâce au mode multi-currency ERPNext.
    if caisse_currency != transaction_currency:
        frappe.throw(
            _(
                "La caisse {0} est en {1}, alors que l'opération "
                "est en {2}. Pour une facture automatique, "
                "la caisse et l'opération doivent utiliser "
                "la même devise."
            ).format(
                doc.caisse,
                caisse_currency,
    ...
    party_account = invoice.get(
        config.party_account_field
    )

    if not party:
        frappe.throw(
            _("Impossible de déterminer le tiers de la facture {0}").format(
                invoice.name
            )
        )
    ...
    if not party_account:
        frappe.throw(
            _(
                "Impossible de déterminer le compte tiers "
                "de la facture {0}"
            ).format(
                invoice.name
            )
    ...
    if not party_currency:
        frappe.throw(
            _(
                "Impossible de déterminer la devise du compte tiers {0}"
            ).format(
                party_account
            )
        )
    ...
    # _convert_amount(...)
    #
    # pour essayer de déterminer allocated_amount.
    #
    # On demande directement à ERPNext l'outstanding qu'il
    # utilisera lui-même pour valider le Payment Entry.
    # ----------------------------------------------------------
    ...
    for reference in outstanding_references:

        reference = frappe._dict(reference)

        if (
            reference.voucher_type == config.invoice_doctype
            and reference.voucher_no == invoice.name
            and flt(reference.outstanding_amount) > 0
        ):
            invoice_references.append(reference)

    if not invoice_references:
        frappe.throw(
            _(
                "Aucun montant restant à payer n'a été trouvé "
                "pour la facture {0}"
            ).format(
                invoice.name
            )
    ...
    # outstanding_amount est maintenant exactement dans
    # la devise attendue par le compte tiers / Payment Ledger.
    party_amount = sum(
        flt(reference.outstanding_amount)
        for reference in invoice_references
    )

    if party_amount <= 0:
        frappe.throw(
            _("La facture {0} est déjà réglée").format(
                invoice.name
            )
        )
    ...
    # ----------------------------------------------------------
    #
    # La facture vient d'être créée depuis cette transaction.
    # Son conversion_rate correspond donc au même événement.
    # ----------------------------------------------------------

    if caisse_currency == company_currency:

        cash_exchange_rate = 1.0

    else:

        cash_exchange_rate = flt(
            invoice.conversion_rate
        )

        if not cash_exchange_rate:
            cash_exchange_rate = _get_currency_rate(
                doc,
                caisse_currency,
                company_currency,
            )
        ...
        if not cash_exchange_rate:
            frappe.throw(
                _(
                    "Aucun taux de change disponible pour convertir "
                    "{0} vers {1}"
                ).format(
                    caisse_currency,
                    company_currency,
    ...
    # Valeur comptable du mouvement de caisse.
    base_cash_amount = (
        transaction_amount
        * cash_exchange_rate
    )
    ...
    # ----------------------------------------------------------
    #
    # Le montant du compte tiers vient du Payment Ledger.
    # On calcule donc le taux qui représente exactement
    # la même valeur comptable que le cash.
    # ----------------------------------------------------------
    ...
    if config.payment_type == "Receive":
        ...
        pe.paid_to_account_currency = caisse_currency

        pe.paid_amount = party_amount
        pe.received_amount = transaction_amount

        pe.source_exchange_rate = party_exchange_rate
        pe.target_exchange_rate = cash_exchange_rate
    ...
    # ----------------------------------------------------------
        ...
        pe.paid_from_account_currency = caisse_currency
        pe.paid_to_account_currency = party_currency

        pe.paid_amount = transaction_amount
        pe.received_amount = party_amount

        pe.source_exchange_rate = cash_exchange_rate
        pe.target_exchange_rate = party_exchange_rate

    # ----------------------------------------------------------
    # REFERENCES
    # ----------------------------------------------------------
    #
    # allocated_amount vient DIRECTEMENT de l'outstanding
    # retourné par ERPNext.
    #
    # Si ERPNext a découpé la facture selon des Payment Terms,
    # on conserve également ces lignes.
    # ----------------------------------------------------------
    ...
    for reference in invoice_references:
        ...
        pe.append(
            "references",
            {
                ...
                "payment_term": (
                    reference.get("payment_term")
                    or ""
                ),
                "exchange_rate": flt(
                    reference.get("exchange_rate")
                    or 1
                ),
            },
        )
...
def make_payment_entry(doc):
    ...
    # ==========================================================
    # FACTURE AUTOMATIQUE SIMPLE
    # ==========================================================
    #
    # prepare_operation() a déjà déterminé qu'il s'agit
    # exclusivement d'une facture créée automatiquement.
    #
    # Aucun recalcul via get_invoice_rows().
    # Aucun get_payment_entry().
    # ==========================================================
    ...
    if (
        auto_invoice
        and doc.flags.get("ls_treso_auto_invoice_only")
    ):
