...
def make_payment_entry(doc):
    ...
    # ==========================================================
    #
    # Cas volontairement limité :
    #
    # - une facture vient d'être créée par create_missing_invoices()
    # - toutes les lignes normales appartiennent à cette facture
    # - aucune ancienne avance n'est utilisée
    # - aucune nouvelle avance
    # - aucun Employee
    #
    # Dans ce cas, ERPNext connaît déjà le vrai outstanding
    # de la facture soumise.
    #
    # On utilise directement get_payment_entry() et surtout
    # on NE reconstruit PAS allocated_amount.
    #
    ...
    if auto_invoice and not doc.advance_allocation:
        ...
        for row in doc.details_operation_de_caisse:
            ...
            # Employé = aucun traitement par facture.
            if row.type_tiers == "Employe":
                auto_invoice_only = False
                break

            # Cas spéciaux = autre traitement.
            if nature.echange or nature.solde_initial:
                auto_invoice_only = False
                break

            # Nouvelle avance = conserver le flux général existant.
            if nature.is_advance:
                auto_invoice_only = False
                break
            ...
            # Si une autre facture est présente dans l'opération,
            # conserver le flux général existant.
        ...
        if has_normal_row and auto_invoice_only:
            ...
            #
            # IMPORTANT :
            # ne pas passer party_amount.
            #
            # ERPNext reprend directement le véritable
            # outstanding de la facture soumise.
            #
            ...
            # pe.set("references", [])
            #
            # PAS de reconstruction de allocated_amount.
            #
            # On garde exactement la référence générée
            # par ERPNext depuis la facture fraîchement soumise.
            #
            pe.submit()
    ...
    if invoice_rows:
        ...
        #
        # Dans le flux général uniquement, plusieurs références
        # peuvent devoir être reconstruites.
        #
        pe.set(
            "references",
            [],
